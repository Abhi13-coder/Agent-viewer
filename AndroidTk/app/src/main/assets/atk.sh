...
fetch() {
    ...
    if command -v curl >/dev/null 2>&1; then
        LD_LIBRARY_PATH="$HOME/lib:$LIB_DIR:$LD_LIBRARY_PATH" curl -fsSL --cacert "$HOME/cacert.pem" -o "$out" "$url"
    else
...
download_deb() {
    ...
    dest="$CACHE_DIR/$base"
    if [ ! -f "$dest" ]; then
        echo "atk: downloading $base" >&2
        fetch "$REPO_BASE/$1" "$dest" || { echo "atk: download failed: $1" >&2; return 1; }
    fi
    echo "$dest"
}
...
extract_deb() {
    ...
    ( cd "$extractdir" && busybox ar -x "$debfile" ) || { echo "atk: ar extract failed" >&2; return 1; }

    datatar=$(ls "$extractdir"/data.tar.* 2>/dev/null | head -n1)
    [ -z "$datatar" ] && { echo "atk: no data.tar.* for $pkgname" >&2; return 1; }
    ...
    case "$datatar" in
        ...
        *.zst) echo "atk: $pkgname uses zstd — unsupported by this busybox" >&2; return 1 ;;
