...
class MainActivity : Activity() {
    ...
    private fun installBundledBusybox() {
        ...
        if (!bundled.exists()) {
            logFile.appendText("busybox missing at ${bundled.absolutePath}\n")
            return
        }
    ...
    /** Generic symlinker for every other plain-named bundled .so (curl's
     * own dependency chain: nghttp2, nghttp3, ngtcp2, ssh2, curl itself). */
    private fun installBundledExtraLibs() {
        val home = filesDir.absolutePath
        val libDir = File(home, "lib").apply { mkdirs() }
        val logFile = File(filesDir, "install.log")
        val handledElsewhere = setOf(
            "libbusybox.so", "libproot.so", "libtalloc.so", "libandroid-shmem.so",
            "libcurl_bin.so", "libz_bundled.so", "libssl_bundled.so", "libcrypto_bundled.so"
        )

        val nativeDir = File(applicationInfo.nativeLibraryDir)
        val libs = nativeDir.listFiles { f -> f.name.startsWith("lib") && f.name.endsWith(".so") } ?: return

        for (bundled in libs) {
            if (bundled.name in handledElsewhere) continue
            bundled.setExecutable(true)
            val link = File(libDir, bundled.name)
            if (!link.exists()) {
                try {
                    Os.symlink(bundled.absolutePath, link.absolutePath)
                } catch (e: Exception) {
                    logFile.appendText("extra-lib symlink failed (${bundled.name}): $e\n")
                }
            }
        }
    }

    /** libz/libssl/libcrypto get bundled under plain renamed names (Android's
     * installer silently drops any native-lib filename with extra dots after
     * ".so", e.g. "libz.so.1.3.2" never reaches nativeLibraryDir at all) —
     * so we symlink the renamed file to the real SONAME the linker expects. */
    ...
    /** Copies the bundled atk.sh out of assets/ into $HOME once. */
    ...
    /** Copies the CA cert bundle out of assets/ into a real file curl can
     * point at directly via --cacert (curl's own compiled-in default path
     * is Termux's private directory, which we can never reach). */
    private fun installBundledCaCert() {
        ...
        try {
            ...
            File(filesDir, "install.log").appendText("cacert.pem asset copy failed: $e\n")
        }
    }
