...
class MainActivity : Activity() {
     ...
     * location Android 10+ still allows executing from. We symlink its
     * applets into $HOME/bin (writable, but the symlinks themselves just
     * point at the real executable location, so they run fine). */
    private fun installBundledBusybox() {
        ...
        val installed = File(home, "bin/busybox")
        if (bundled.exists() && !installed.exists()) {
            try {
                ProcessBuilder(bundled.absolutePath, "--install", "$home/bin").start().waitFor()
            } catch (e: Exception) {
                File(filesDir, "install.log").appendText("busybox --install failed: ${e.message}\n")
            }
        }
    }

    /** Seeds a default prompt once. After this, the user can permanently
     * change it themselves by editing $HOME/.profile (same convention as
     * Termux's .bashrc) — e.g. `echo "export PS1='me\$ '" >> $HOME/.profile` */
    private fun ensureDefaultProfile() {
        val profile = File(filesDir, ".profile")
        if (!profile.exists()) {
            profile.writeText("export PS1='Atk\$ '\n")
        }
    }
    ...
    private fun createSession(): TerminalSession {
        ...
        // apply any user customization from .profile (e.g. a changed PS1)
        session.write("[ -f \"\$HOME/.profile\" ] && . \"\$HOME/.profile\"\n")
